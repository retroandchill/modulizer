...
impl<'a, W: Write> SymbolWriteContext<'a, W> {
    ...
    pub fn emit_symbol(&mut self, symbol: &Symbol) -> std::io::Result<()> {
        ...
        match &symbol.kind {
            SymbolKind::Namespace(namespace) => {
                ...
                self.writer.write_fmt(format_args!("namespace {}\n", namespace.name))?;
    ...
    fn update_guards(&mut self, new_guards: &[PreprocessorGuard]) -> std::io::Result<()> {
        ...
        let can_switch_to_else =
            self.guards.len() == shared_prefix_len + 1
                && new_guards.len() == shared_prefix_len + 1
                && matches!(self.guards.back(), Some(PreprocessorGuard::Conditional(_)))
                && matches!(new_guards.get(shared_prefix_len), Some(PreprocessorGuard::Conditional(ConditionalDirective::Elif { .. } | ConditionalDirective::Elifdef { .. } | ConditionalDirective::Elifndef { .. }) |PreprocessorGuard::Else));
        ...
        for guard in &new_guards[shared_prefix_len..] {
            match guard {
                PreprocessorGuard::Conditional(condition) => {
                    match condition {
                        ConditionalDirective::If { expression } => {
                            self.writer.write_all_unindented(b"#if ")?;
                            for token in expression {
                                self.writer.write_fmt_unindented(format_args!("{}", token))?;
                            }
                            self.writer.write_all_unindented(b"\n")?;
                        }
                        ConditionalDirective::Ifdef { name } => {
                            self.writer.write_fmt_unindented(format_args!("#ifdef {}\n", name))?;
                        }
                        ConditionalDirective::Ifndef { name } => {
                            self.writer.write_fmt_unindented(format_args!("#ifndef {}\n", name))?;
                        }
                        ...
                        ConditionalDirective::Elifdef { name } => {
                            self.writer.write_fmt_unindented(format_args!("#elifdef {}\n", name))?;
                        }
                        ConditionalDirective::Elifndef { name } => {
                            self.writer.write_fmt_unindented(format_args!("#elifndef {}\n", name))?;
                        }
                    }
                }
