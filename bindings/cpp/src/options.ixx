...
namespace modulizer
{
    ...
    export class Options
    {
        ...
        constexpr explicit Options(Modulizer_Options *options) noexcept : options_{options}
        {
        }
      ...
      public:
        ...
        decltype(auto) include_dirs(this Self &&self, Range &&paths)
        {
            auto path_vector = std::forward<Range>(paths) |
                               std::views::transform([](const auto &path) { return to_c(path); }) |
                               std::ranges::to<std::vector>();
            modulizer_builder_include_dirs(self.builder.get(), path_vector.data(), path_vector.size());
            return std::forward<Self>(self);
        }
        ...
        {
            modulizer_builder_expand_macro_from_definition(self.builder.get(), to_c(name));
            return std::forward<Self>(self);
        }
        ...
        decltype(auto) expand_macros_from_definition(this Self &&self, Range &&names)
        {
            auto name_vector = std::forward<Range>(names) |
                               ...
                               std::ranges::to<std::vector>();
            modulizer_builder_expand_macros_from_definition(self.builder.get(), name_vector.data(), name_vector.size());
            return std::forward<Self>(self);
        }
        ...
        decltype(auto) explicit_macros(this Self &&self, Range &&names)
        {
            auto definition_vector = std::forward<Range>(names) |
                                     ...
                                     std::ranges::to<std::vector>();
            modulizer_builder_explicit_macros(self.builder.get(), definition_vector.data(), definition_vector.size());
            return std::forward<Self>(self);
        }
