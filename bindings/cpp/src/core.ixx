...
namespace modulizer
{
    ...
    export using IncludePath = std::variant<std::string_view, IfdefInclude, ConditionalInclude>;

    constexpr Modulizer_IncludePath to_c(const IncludePath &path)
    {
        return std::visit(
            Overload{[](std::string_view path) {
                         return Modulizer_IncludePath{.kind = MODULIZER_INCLUDE_TYPE_UNCONDITIONAL,
                     ...
                     [](const IfdefInclude &include)
                     {
                         return Modulizer_IncludePath{
                             ...
                             .ifdef = {.path = to_c(include.path), .if_defined = to_c(include.if_defined)}};
                     },
                     [](const ConditionalInclude &include)
                     {
                         return Modulizer_IncludePath{
                             .kind = MODULIZER_INCLUDE_TYPE_CONDITIONAL,
                             .conditional = {.path = to_c(include.path), .condition = to_c(include.condition)}};
                     }},
            path);
    }
} // namespace modulizer
